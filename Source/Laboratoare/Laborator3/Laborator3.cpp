...
Mesh* CreateTriangle(std::string name)
{
	...
	std::vector<VertexFormat> triangleVertices =
	{
		...
		VertexFormat(glm::vec3(1, 0.5, 0), glm::vec3(0, 1, 1))
	};
...
void Laborator3::Init()
{
	...
	Mesh* triangle = CreateTriangle("triangle");
...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	//angularStep = angularStep + 1.5 * deltaTimeSeconds;
	//modelMatrix *= Transform2D::Rotate(angularStep);
	//modelMatrix *= Transform2D::Translate(5, 300);
	RenderMesh2D(meshes["triangle"], shaders["VertexColor"], modelMatrix);

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(450, 380);
